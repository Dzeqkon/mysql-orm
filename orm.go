...
func (orm *ORMGenerator) buildORMImport() {
	importBuilder := dzgutils.NewStringBuilder()
...
func (orm *ORMGenerator) buildORMStruct(tabName string, ORMTab ORMTable, hasComment bool) {
	...
	fieldNames := dzgutils.NewInterfaceBuilder()
	for k := range cols {
		col := cols[k]
		colName := col.CName
		...
		fieldName := dzgutils.FirstCaseToUpper(colName, true)
	...
	structBuilder.Append("\n")
	structBuilder.Append("}").Append("\n")
	dzgutils.Stdout(structBuilder.ToString())
...
func (orm *ORMGenerator) buildORMSqlSelect(tabName string, cols []ORMColumn) {
	...
	funcRowBuilder := dzgutils.NewStringBuilder()
	...
	funcRowBuilder.Append("\t").Append("builder := dzgutils.NewInterfaceBuilder()").Append("\n")
	for i := range fieldNamesArray {
		funcRowBuilder.Append("\t").Append("builder.Append(&").Append(aliasStructName).Append(".").Append(dzgutils.InterfaceToString(fieldNamesArray[i])).Append(")").Append("\n")
	}
	...
	funcRowsBuilder.Append("\t").Append("builder := dzgutils.NewInterfaceBuilder()").Append("\n")
	funcRowsBuilder.Append("\t").Append("for rows.Next() {").Append("\n")
	funcRowsBuilder.Append("\t\t").Append("builder.Clear()").Append("\n")
	for i := range fieldNamesArray {
		funcRowsBuilder.Append("\t\t").Append("builder.Append(&").Append(aliasStructName).Append(".").Append(dzgutils.InterfaceToString(fieldNamesArray[i])).Append(")").Append("\n")
	}
	...
	dzgutils.Stdout(funcRowsBuilder.ToString())
}

func (orm *ORMGenerator) buildORMSqlInsert(tabName string) {
	funcInsertBuilder := dzgutils.NewStringBuilder()
	structName := getStructName(tabName)
	aliasStructName := getAliasStructName(tabName)
	...
	funcInsertBuilder.Append("\t").Append("sql := dzgutils.NewStringBuilder()").Append("\n")
	funcInsertBuilder.Append("\t").Append("qSql := dzgutils.NewStringBuilder()").Append("\n")
	funcInsertBuilder.Append("\t").Append("params := dzgutils.NewInterfaceBuilder()").Append("\n")
	...
	dzgutils.Stdout(funcInsertBuilder.ToString())
}

func (orm *ORMGenerator) buildORMSqlUpdate(tabName string) {
	funcUpdateBuilder := dzgutils.NewStringBuilder()
	structName := getStructName(tabName)
	aliasStructName := getAliasStructName(tabName)
	...
	funcUpdateBuilder.Append("\t").Append("sql := dzgutils.NewStringBuilder()").Append("\n")
	funcUpdateBuilder.Append("\t").Append("params := dzgutils.NewInterfaceBuilder()").Append("\n")
	...
	dzgutils.Stdout(funcUpdateBuilder.ToString())
}

func (orm *ORMGenerator) buildORMSqlDelete(tabName string) {
	funcDeleteBuilder := dzgutils.NewStringBuilder()
	structName := getStructName(tabName)
	aliasStructName := getAliasStructName(tabName)
	...
	funcDeleteBuilder.Append("\t").Append("sql := dzgutils.NewStringBuilder()").Append("\n")
	...
	dzgutils.Stdout(funcDeleteBuilder.ToString())
}

func (orm *ORMGenerator) buildORMSqlBatchInsert(tabName string) {
	funcBatchInsertBuilder := dzgutils.NewStringBuilder()
	structName := getStructName(tabName)
	aliasStructName := getAliasStructName(tabName)
	structNames := getStructNames(tabName)
	...
	funcBatchInsertBuilder.Append("\t").Append("sql := dzgutils.NewStringBuilder()").Append("\n")
	funcBatchInsertBuilder.Append("\t").Append("oneQSql := dzgutils.NewStringBuilder()").Append("\n")
	funcBatchInsertBuilder.Append("\t").Append("batchQSql := dzgutils.NewStringBuilder()").Append("\n")
	...
	funcBatchInsertBuilder.Append("\t").Append("batchInsertColsLen := dzgutils.InterfaceToInt(dzgutils.IIIInterfaceOperator(idSet, fieldsNum, fieldsNum-1))").Append("\n")
	...
	funcBatchInsertBuilder.Append("\t\t").Append("batchSql := dzgutils.NewStringBuilder().Append(sql.ToString()).Append(batchQSql.ToString()).Append(\";\").ToString()").Append("\n")
	funcBatchInsertBuilder.Append("\t\t").Append("batchParams := dzgutils.NewInterfaceBuilder()").Append("\n")
	...
	funcBatchInsertBuilder.Append("\t\t").Append("oneSql := dzgutils.NewStringBuilder().Append(sql.ToString()).Append(oneQSql.ToString()).Append(\";\").ToString()").Append("\n")
	funcBatchInsertBuilder.Append("\t\t").Append("oneParams := dzgutils.NewInterfaceBuilder()").Append("\n")
	...
	dzgutils.Stdout(funcBatchInsertBuilder.ToString())
}

func getStructName(tabName string) string {
	return dzgutils.FirstCaseToUpper(tabName, true)
}

func getAliasStructName(tabName string) string {
	return dzgutils.FirstCaseToUpper(tabName, false)
}

func getStructNames(tabName string) string {
	return dzgutils.NewString(getStructName(tabName)).AppendString("s").ToString()
}

func getAliasStructNames(tabName string) string {
	return dzgutils.NewString(getAliasStructName(tabName)).AppendString("s").ToString()
}
...
func (orm *ORMGenerator) getDbInfo() {
	...
	for rows.Next() {
		...
		dzgutils.CheckAndPrintError("Get db info rows scan failed", err)
		setORMTabsCols(tName, tComment, cName, getDBType(cType), cComment)
	}
}

func getDBType(cType string) string {
	typeTmp := dzgutils.NewString(cType)
	if typeTmp.Contains(dzgutils.PARENTHESIS_LEFT) {
		return typeTmp.SubstringEnd(typeTmp.Index(dzgutils.PARENTHESIS_LEFT)).ToUpper().ToString()
	}
