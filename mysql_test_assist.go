...
import (
	db "database/sql"
	"errors"
	dzgutils "github.com/Dzeqkon/goutils"
	"reflect"
	"time"
)
...
func (weTestTab1 *WeTestTab1) RowToStruct(row *db.Row) error {
	builder := dzgutils.NewInterfaceBuilder()
...
func (weTestTab1 *WeTestTab1) Insert(client *DBClient, idSet bool) (int64, error) {
	...
	sql := dzgutils.NewStringBuilder()
	qSql := dzgutils.NewStringBuilder()
...
func (weTestTab1 *WeTestTab1) BatchInsert(client *DBClient, idSet, returnIds bool) ([]int64, error) {
	...
	sql := dzgutils.NewStringBuilder()
	oneQSql := dzgutils.NewStringBuilder()
	batchQSql := dzgutils.NewStringBuilder()
	ks := reflect.TypeOf(structParam)
	...
	batchInsertColsLen := dzgutils.InterfaceToInt(dzgutils.IIIInterfaceOperator(idSet, fieldsNum, fieldsNum-1))
	oneQSql.Append("(")
	...
	if !returnIds {
		...
		batchSql := dzgutils.NewStringBuilder().Append(sql.ToString()).Append(batchQSql.ToString()).Append(";").ToString()
		batchParams := dzgutils.NewInterfaceBuilder()
		for k := range list {
			item := list[k]
		...
		oneSql := dzgutils.NewStringBuilder().Append(sql.ToString()).Append(oneQSql.ToString()).Append(";").ToString()
		oneParams := dzgutils.NewInterfaceBuilder()
