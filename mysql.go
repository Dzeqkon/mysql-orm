...
func (client *DBClient) QueryRow(orm ORMBase, sql string, args ...interface{}) (row *db.Row, err error) {
	...
	client.slowSql(dzgutils.Millisecond()-start, sql, args...)
...
func (client *DBClient) TxCommit(tx *db.Tx) bool {
	err := tx.Commit()
	if err != nil {
		dzgutils.CheckAndPrintError(MySQL+" tx commit failed", err)
		return false
	}
	return true
}
...
func (client *DBClient) TxRollback(tx *db.Tx) {
	err := tx.Rollback()
	if err != nil {
		dzgutils.CheckAndPrintError(MySQL+" tx rollback failed", err)
	}
}
...
func (client *DBClient) QueryMetaData(tabName string) *db.Rows {
	...
	dzgutils.CheckAndPrintError("Query '"+tabName+"' table meta data failed", err)
	return rows
}
...
func (client *DBClient) QueryDBInfo() *db.Rows {
	...
	dzgutils.CheckAndPrintError("Query '"+client.Config.DbName+"' db info failed", err)
	return rows
}
...
func (client *DBClient) forkExec(stmt *db.Stmt, sql string, args ...interface{}) (result int64, err error) {
	...
	var intResult int64
	if dzgutils.NewString(sql).ContainsIgnoreCase("INSERT") {
